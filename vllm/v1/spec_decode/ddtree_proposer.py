    ...
    def __init__(
        ...
        if self._ddtree_verify_enabled:
            logger.info(
                ...
                "S1+S2+S3b+S3c-1+S3c-2+S3c-3+S3c-4 path active. drafter "
                "runs at horizon=15 internally; propose() returns "
                "budget tree-node tokens; the visibility mask is "
                "attached to target's TritonAttentionMetadata."
                "tree_attention_mask and applied by the triton kernel "
                "as additive qq_bias; accept uses greedy tree-follow "
                "over target's argmax at each verify position; AFTER "
                "accept the per-request KV cache is compacted so the "
                "scattered tree-path accepts become a contiguous "
                "prefix for the next round (S3c-4). Acceptance length "
                "expected > dflash baseline of 3.228.",
                self.ddtree_budget,
